"use client";
// Home list row — compact view with edit button, click anywhere to navigate
...
export default function HomeListRow({ id, name, address, houseManagerName, residentCount, flaggedCount, bedCount, onRefresh }: HomeListRowProps) {
  ...
  return (
    <div
      ...
      className="flex items-center gap-4 bg-white rounded-xl px-4 py-3.5 card-hover cursor-pointer border group"
      ...
      <div className="flex items-center gap-4 flex-shrink-0">
        ...
        {vacancies !== null && (
          <div className="flex items-center gap-1.5 text-sm">
            ...
            <span className="font-semibold" style={{ color: vacancies === 0 ? "#DC2626" : "#0B1F3A" }}>
              {vacancies}
            </span>
          </div>
        )}
