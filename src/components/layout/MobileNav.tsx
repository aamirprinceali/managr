...
export default function MobileNav() {
  ...
  return (
    <nav
      ...
      style={{ background: "#FFFFFF", borderTop: "1px solid #E2E8F0", boxShadow: "0 -1px 8px rgba(0,0,0,0.06)" }}
    >
      <div className="flex items-center h-16 px-1">
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              <div
                className={cn("w-10 h-6 flex items-center justify-center rounded-lg transition-all duration-150")}
                style={isActive ? { background: "#DBEAFE" } : {}}
              >
                <Icon
                  size={20}
                  strokeWidth={isActive ? 2.5 : 1.8}
                  style={{ color: isActive ? "#1B6EF3" : "#94A3B8" }}
                />
              </div>
              <span
